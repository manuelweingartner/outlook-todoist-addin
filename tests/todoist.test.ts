...
describe("getTasksByFilter", () => {
  ...
  test("folgt next_cursor ueber mehrere Seiten", async () => {
    const fn = jest.fn()
      .mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({ results: [{ id: "1", content: "A", project_id: "p" }], next_cursor: "c2" }), text: async () => "" })
      .mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({ results: [{ id: "2", content: "B", project_id: "p" }], next_cursor: null }), text: async () => "" });
    (global as any).fetch = fn;
    const tasks = await getTasksByFilter("tok", "today");
    expect(tasks.map((t) => t.id)).toEqual(["1", "2"]);
    expect(fn.mock.calls[1][0]).toContain("cursor=c2");
  });

  test("wirft TodoistError bei Fehler", async () => {
    mockFetch(400, { error: "bad query" });
    await expect(getTasksByFilter("tok", "??")).rejects.toBeInstanceOf(TodoistError);
  });
});
